...
/*
            ...
            for (uint32_t j = 0; j < static_cast<uint32_t>(digiView.metadata().size()); j++) {
                if ( static_cast<uint32_t>(digiView.moduleId(j)) == moduleId) {
                    if ( static_cast<uint32_t>(digiView.clus(j)) == clusterOffset) {                        
                        originalpixels_x[ kk ] = digiView.xx(j); // Copy x-coordinate from original pixel
                        originalpixels_y[ kk ] = digiView.yy(j); // Copy y-coordinate from original pixel
                        kk++;
                    }
                }
            }
            ...
            if (split) {
                ...
                if (meanExp <= 1) {
                    ...
                    for (uint32_t jj = begin; jj < end; ++jj) {
                        ...
                        int sub = static_cast<int>(digiView.adc(jj)) / chargePerUnit_ * expectedADC / centralMIPCharge_;
                        if (sub < 1) sub = 1;

                        int perDiv = digiView.adc(jj) / sub;
                        ...
                        for (int k = 0; k < sub; ++k) {
                            ...
                            rawIdArr[pixelsSize]    = digiView.rawIdArr(jj);
                    ...
                    while (!stop && remainingSteps > 0) {
                        ...
                        // SORT SCORES ------------------------------------
                        for (uint32_t i = 0; i < pixelCounter - 1; i++) {
                            for (uint32_t j = 0; j < pixelCounter - i - 1; j++) {
                        ...
                        if (verbose_) {
                            printf("Scores:\n");
                            for (uint32_t k = 0; k < pixelCounter; k++) {
                        ...
                        for (unsigned int i = 0; i < pixelCounter; i++) {
                            if (i < maxPixels) {
                                int pixel_index = scoresIndices[i];
                                ...
                                for (unsigned int subpixel = 0; subpixel < pixelsSize; subpixel++, subpixel_counter++) {
                                    if (subpixel < maxPixels) {
                                        if (pixels[subpixel] > static_cast<uint32_t>(pixel_index)) {
                                            break;
                                        } else if (pixels[subpixel] != static_cast<uint32_t>(pixel_index)) {
                                            continue;
                                        } else {
                                            float maxEst = 0.f;
                                            int cl = -1;
                                            ...
                                            // On-the-fly coordinate lookup for this pixel
                                            float temp_originalpixels_x = 0.f;
                                            float temp_originalpixels_y = 0.f;

                                            uint32_t begin = geoclusterView.pixelStart(clusterIdx);
                                            uint32_t end = begin + geoclusterView.pixelCount(clusterIdx);

                                            for (uint32_t jj = begin, matchIdx = 0; jj < end; ++jj, ++matchIdx) {
                                                if (matchIdx == static_cast<uint32_t>(pixel_index)) {
                                                    temp_originalpixels_x = digiView.xx(jj);
                                                    temp_originalpixels_y = digiView.yy(jj);
                                                    break;
                                                }
                                            ...
                                            // Loop over subclusters to compute score
                                            for (unsigned int subcluster_index = 0; subcluster_index < meanExp; subcluster_index++) {
                                                if (subcluster_index < maxSubClusters) {

                                                    float distanceX = temp_originalpixels_x - clx[subcluster_index];
                                                    float distanceY = temp_originalpixels_y - cly[subcluster_index];
                    ...
                    uint32_t globalOffset = alpaka::atomicAdd(acc, clusterCounterDevice, uint32_t(0));
                    ...
                    for (int cl = 0; cl < static_cast<int>(meanExp); cl++) {
                        ...
                        for (unsigned int j = 0; j < pixelsSize; j++) {
                            if (j < maxPixels) {
                                if (clusterForPixel[j] == cl &&
                                    ...
                                    // Accumulate ADC from overlapping pixels
                                    ...
                                    uint32_t outIdx = globalOffset + pixelOffset;

                                    if (verbose_) printf("Pixels of cl=%d ",cl);
                                    if (verbose_) printf("pixel_X[%d].x=%d, pixel_Y[%d].y=%d, pixel_ADC[%d].adc=%d\n",
                                           j, pixel_X[j],
                                           j, pixel_Y[j],
                                           j, pixel_ADC[j]);

                                    outputDigis.clus(outIdx) = cl;
                                    outputDigis.xx(outIdx) = static_cast<uint16_t>(pixel_X[j]);
                                    outputDigis.yy(outIdx) = static_cast<uint16_t>(pixel_Y[j]);
                                    outputDigis.adc(outIdx) = static_cast<uint16_t>(pixel_ADC[j]);
                                    outputDigis.rawIdArr(outIdx) = static_cast<uint32_t>(rawIdArr[j]);
                        ...
                        // Advance cluster counter after finishing current subcluster
                        globalOffset = alpaka::atomicAdd(acc, clusterCounterDevice, uint32_t(1));
                    }
                }
            }
        }
